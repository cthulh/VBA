...
With Sheet1
    ...
    For counter = report_start To lastrow
        If Len(Trim(.Cells(counter, 1).Value)) = 6 Then
            ' Route number
...
With Sheet2
    ...
    counter = 3
    ' Clear report
    .Range(.Cells(3, 1), .Cells(10000, 2)).ClearContents
    .Range(.Cells(3, 4), .Cells(10000, 6)).ClearContents
...
With Sheet2
    lastrow = .Cells(Rows.Count, 1).End(xlUp).Row
    ' Report starts from row 3, slip first row
    .ResetAllPageBreaks
